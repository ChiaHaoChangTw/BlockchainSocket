...
#define PORT_M "25268" //the port client will be connecting to 
...
int main(int argc, char *argv[]){
    ...
    //printf("clientA: connecting to %s\n", s);
...
void handleCheckOp(int sockfd, char* argv[]){
	//send()
    string messageSend = "A CHECK " + string(argv[1]) + "\n";
...
//handle TXCOINS request
void handleTxOp(int sockfd, char* argv[]){
	//send() - A TXCOINS username1 username2 amount
    string messageSend = "A TXCOINS " + string(argv[1]) + " " + 
...
//handle stats request
void handleStatsOp(int sockfd, char* argv[]){
	//send() - A stats username
    string messageSend = "A stats " + string(argv[1]) + "\n";
    if(send(sockfd, messageSend.c_str(), messageSend.size() + 1, 0) == -1){
		perror("send");
		exit(1);
	}
